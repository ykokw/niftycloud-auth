...
class V4 extends Client{
  ...
  get(path, header, query, serviceId, region, cb) {
    const requestParams = {
      path: path,
      method: "get",
      header: header,
  ...
  post(path, header, query, body, serviceId, region, cb) {
    const requestParams = {
      path: path,
      method: "post",
      header: header,
  ...
  //  req.header["Authorization"] = this.createAuthorizationHeader(signatureObject, signature);
