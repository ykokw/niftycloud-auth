...
class V2 extends Client{
  ...
  get(path, action = "", queries = {}, cb) {
    if (typeof(path) !== "string") return this.handleError(new this.InvalidParametersError("path must be string"), cb);
    if (typeof(action) !== "string") return this.handleError(new this.InvalidParametersError("action must be string"), cb);
    if (typeof(queries) !== "object") return this.handleError(new this.InvalidParametersError("queries must be string"), cb);
    const method = "get";
