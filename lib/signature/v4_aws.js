...
class V4AWS extends Client{
  ...
  stringToSign(opt) {
    if (opt.headers && opt.headers.hasOwnProperty("X-Amz-Date")) {
      opt.requestDate = opt.headers["X-Amz-Date"];
    } else {
      return null;
    }
  ...
  createSecret(opt) {
    const hmacForDate = crypto.createHmac("sha256", `AWS4${this.secretKey}`);
    ...
    hmacForSecret.update("aws4_request");
    return hmacForSecret.digest();
  }

  createCanonicalScope(opt) {
    return `${opt.requestDate.substr(0, 8)}/${opt.region}/${opt.serviceId}/aws4_request`;
  }
  ...
  createAuthorizationHeader(params, signature){
    return `AWS4-HMAC-SHA256 Credential=${this.accessKey}/${params.canonicalScope}, SignedHeaders=${Utils.createSignedHeaderString(params.headers)}, Signature=${signature}`;
  }

  sendRequest(req){
    if (req === null) return Promise.reject(new Errors.InvalidParameterError("request object must not be null."));
    req.headers["X-Amz-Date"] = moment().format("YYYYMMDDTHHmmss") + "Z";
